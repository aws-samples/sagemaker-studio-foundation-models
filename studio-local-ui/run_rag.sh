...
# Run the Streamlit app and save the output to "temp_rag.txt"
streamlit run rag_app.py --server.runOnSave true > temp_rag.txt & 
...
sleep 5
PORT=$(grep "Network URL" temp_rag.txt | awk -F':' '{print $NF}' | awk '{print $1}' | tail -c 5)
